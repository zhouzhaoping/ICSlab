...
void eval(char *cmdline) 
{
	...
	if (!builtin_command(&tok))            //not a builtin_command
	{                                 
		sigemptyset(&mask);
		sigaddset(&mask, SIGCHLD);
		sigaddset(&mask, SIGINT);
		sigaddset(&mask, SIGTSTP);
		sigprocmask(SIG_BLOCK, &mask, NULL);      /*block,处理竞争条件*/
		...
		if ((pid = fork()) == 0)
		{
			...
			if (execve(tok.argv[0], tok.argv, environ) < 0)  //error check execve
			{
...
/*
* if it is a built-in command then execute it immediately.   
*/
int builtin_command(struct cmdline_tokens *tok)
{
	//quit command
	...
	if (tok->builtins == BUILTIN_JOBS)
	{
		...
		if (tok->outfile)                               //I/O redirection 
			fd = open(tok->outfile, O_WRONLY, 0);

		listjobs(job_list, fd);                        //show job_list into tok->outfile
		return 1;
	}
...
/*  
* bg_or_fg - Execute the builtin bg and fg commands 
*/
void bg_or_fg(char **argv)
{
	struct job_t *job;

	if (argv[1][0] == '%')                              //it is a jid
		job = getjobjid(job_list, atoi(argv[1] + 1));
	else                                                //it is a pid
		job = getjobpid(job_list, atoi(argv[1]));
	
	if (job == NULL)                                    //job error
	{
		printf("job is not exit\n");
		fflush(stdout);
		return;
	}

	if (!(strcmp("bg", argv[0])))                      //bg
	{
		job->state = BG;
		printf("[%d] (%d) %s\n", job->jid, job->pid, job->cmdline);
		fflush(stdout);
		kill(job->pid, SIGCONT);                       //restart
	}
	else                                               //fg
	{
		job->state = FG;
		kill(job->pid, SIGCONT);                        //restart
		wait_fg(job->pid);                              //than wait the fg
	}
	return;
}
/*  
...
void wait_fg(pid_t pid)
{
	...
	if (job == NULL)                            //pid is not fg, wait_fg is no need
		return;

	while (job->pid == pid && job->state == FG)  //the fg still running fg
		usleep(1000);
...
void sigchld_handler(int sig) 
{
	...
	if (verbose) printf("sigchld_handler: \n");

	while ((pid = waitpid(-1, &status, WUNTRACED | WNOHANG)) > 0)//wait
	{
		if (verbose) printf("sigchld_handler: entering\n");
		   
		if (WIFSTOPPED(status))                    //sends a SIGCHLD because of stop
		{
		...
		else                                        //sends a SIGCHLD because of terminate
		{
			if (verbose) printf("sigchld_handler: Job [%d] (%d) deleted\n", pid2jid(pid), pid);
			
			if (WIFSIGNALED(status))                //terminate because of signal
			{ 
				printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(pid), pid, WTERMSIG(status));
				fflush(stdout);
			}
			else                                   //terminate normally
...
void sigint_handler(int sig) 
{
	...
	if (pid != 0)
	{
		...
		kill(-pid, SIGINT);  //sent SIGINT to pid's group
	}
	return;
}
...
void sigtstp_handler(int sig) 
{
	...
	if (pid != 0)
	{
		...
		kill(-pid, SIGTSTP); //sent SIGTSTP to pid's group
	}
	return;
}
